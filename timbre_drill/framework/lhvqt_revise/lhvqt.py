...
class LHVQT(torch.nn.Module):
    ...
    def get_pd1(self, index):
        # Obtain a pointer to the lower-level modules
        lvqt_modules = self.get_modules()[index]

        return lvqt_modules.pd1
    
    def get_sd1(self, index):
        # Obtain a pointer to the lower-level modules
        lvqt_modules = self.get_modules()[index]

        return lvqt_modules.sd1
    
    def get_ks1(self, index):
        lvqt_modules = self.get_modules()[index]

        return lvqt_modules.ks1
    
    def LHVQT_post_proc(self, feats):
        lvqt_modules = self.get_modules()[1]
        feats = lvqt_modules.post_proc(feats)
    ...
    def forward(self, wav):
        ...
        # Loop through harmonics
        for h in range(len(self.harmonics)):
            if self.harmonics[h] == 1:
              # Take the transform at each harmonic
              tf, complex_tf = lvqt_modules[h](wav, True)
