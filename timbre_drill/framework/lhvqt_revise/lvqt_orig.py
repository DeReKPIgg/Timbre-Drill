...
class LVQT(_LVQT):
    ...
    def forward(self, audio, IF_basis_harmonic=False):
        """
        ...
        if IF_basis_harmonic:
          feats_real = feats[: ,::2, :].unsqueeze(1)
          feats_imag = feats[:, 1::2, :].unsqueeze(1)
          feats_sep = torch.cat([feats_real, feats_imag], dim=1)

          # Perform post-processing steps
          feats_sep = self.post_proc(feats_sep)
