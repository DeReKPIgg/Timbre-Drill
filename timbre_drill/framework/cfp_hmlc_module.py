...
class CFP_HMLC():
    ...
    def get_window(self):
        return scipy.signal.windows.blackmanharris(self.win)
    ...
    def STFT_complex(self, x, h):      
        ...
        t = np.arange(1, np.ceil(x.size(-1)/float(self.hop))*self.hop, self.hop)
        ...
        tfr = torch.from_numpy(np.zeros((x.size(0), int(N), len(t)), dtype=np.float64)).to(device)

        for ic in range(x.size(0)): 
            for icol in range(0, len(t)):
                ti = int(t[icol])           
                tau = np.arange(int(-min([round(N/2.0)-1, Lh, ti-1])), \
                                int(min([round(N/2.0)-1, Lh, x.size(-1)-ti])))
                indices = np.mod(N + tau, N) + 1                                             
                tfr[ic, indices-1, icol] = x[ic][ti+tau-1] * h[Lh+tau-1] #\
        #                                /np.linalg.norm(h[Lh+tau-1])

        tfr = torch.fft.fft(tfr, n=N, dim=1)
    ...
    def dwindow(self, h):
        ...
        L = np.arange(-Lh,Lh+1,1, dtype=np.float64)
        L_T = L.reshape(-1,1)
    ...
    def Freq2LogFreqMapping(self, tfr, f):
        ...
        freq_band_transformation = np.zeros((Nest, len(f)), dtype=np.float64)
        for i in range(self.bov, Nest-self.bov):
    ...
    def CFP_filterbank(self, x):
        ...
        h = torch.from_numpy(self.get_window()).to(device)

        tfr, f, t, N = self.STFT_magnitude(x, h)
        tfr = torch.pow(torch.abs(tfr), self.g[0])
        tfr0 = tfr # original STFT
        ceps = torch.zeros_like(tfr)

        if NumofLayer >= 2:
            for gc in range(1, NumofLayer):
                if np.remainder(gc, 2) == 1:
                    tc_idx = round(self.fs*self.tc)
                    ceps = torch.real(torch.fft.fft(tfr, axis=1)) / np.sqrt(N)
                    ceps = nonlinear_func(ceps, self.g[gc], tc_idx)
                else:
                    fc_idx = round(self.fc/self.fr)
                    tfr = torch.real(torch.fft.fft(ceps, axis=1)) / np.sqrt(N)
                    tfr = nonlinear_func(tfr, self.g[gc], fc_idx)
        ...
        tfrL0_batch, tfrLF_batch, tfrLQ_batch = [], [], []

        #tfrL0, central_frequencies = self.Freq2LogFreqMapping(tfr0, f)

        # for i in range(tfr.size(0)):
        #     tfrLF, central_frequencies = self.Freq2LogFreqMapping(tfr[i].cpu(), f)
        #     tfrLF_batch.append(tfrLF)

        #tfrLQ, central_frequencies = self.Quef2LogFreqMapping(ceps, q)

        for i in range(tfr.size(0)):
            tfrLQ, central_frequencies =self.Quef2LogFreqMapping(ceps[i].cpu(), q)
            tfrLQ_batch.append(tfrLQ)
        
        # tfrLF_batch = torch.from_numpy(np.array(tfrLF_batch)).to(device)
        tfrLQ_batch = torch.from_numpy(np.array(tfrLQ_batch)).to(device)

        #return tfrL0, tfrLF, tfrLQ, f, q, t, central_frequencies 
        return tfrLQ_batch, f, q, t, central_frequencies 
...
class Note_level_processing():    
    ...
    #    SF = np.convolve(SF, np.ones(21)/21.0, mode='same')
        _, onset = findpeaks_time(SF, self.SF_thres)
        return SF, onset
    ...
    def note_seg_dp(self, Z, onset_time):
        ...
        for i in range(len(onset_time)-1):
            ...
            Z_[Z_<0]=0
            _, contour, contour_val = DPNT(Z_, self.lam)
