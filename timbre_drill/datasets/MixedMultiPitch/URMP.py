...
class URMP(MPEDataset, URMP):
    ...
    def get_note_ground_truth(self, track):
        """
        Extract the ground-truth for the specified track.
        ...
        # Initialize ground-truth variables
        intervals, pitches = None, None

        # Loop through instruments
        for instrument in instruments:
            # Obtain the path to the ground_truth for the instrument
            txt_path = self.get_ground_truth_path(track, instrument)
            txt_path = txt_path.replace('F0s', 'Notes')
            ...
            # Break apart frame time and pitch observations from annotations
            _onset_times, _pitches, duration = np.array(annotations).astype('float').T
            _offset_times = _onset_times + duration

            _onset_times = np.expand_dims(_onset_times, axis=0)
            _offset_times = np.expand_dims(_offset_times, axis=0)

            _interval = np.concatenate((_onset_times, _offset_times)).T
            ...
            if intervals is None or pitches is None:
                # Use provided times and pitches
                intervals, pitches, = _interval, _pitches
            else:
                # Append the pitch observations
                pitches = np.append(pitches, _pitches, axis=1)
                intervals = np.append(intervals, _interval, axis=0)
